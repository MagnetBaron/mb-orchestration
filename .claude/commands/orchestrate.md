...
5. **Route reviews by the router (which reads `bin/usage-status.py`), not guesswork.** Order: **Fable → Codex Sol → Opus 4.8 → Review E (if wired) → stop**. Cross-family = one pass each from two families (Fable + 4.8 are one family). Exhaustion / Review E / quota-vs-outage rules live in `AGENTS.md` + `fireworks-usage.md`.
