...
Prefer another implementer when usable. Implementers/authors cannot review their own artifact. Effective dispatcher may review an artifact it did not author, but that pass is artifact-only on a separate physical invocation; another reviewer independently checks dispatch intent/risk and the intake family must not be the sole reviewer. Ordinary minimum-necessary repo artifacts are preauthorized by `config/handoff-policy.json` `standing_review_authorization` (all configured review providers; no per-review approval); restricted/unknown data parks without a permission loop.

Contract: read the authoritative checkout's `AGENTS.md`; for routing, `config/review-depth.json` (machine floor) with `DOCTRINE.md` §Review depth (the human explanation). Live model/route identity is `config/model-registry.json` via `bin/model-registry.py` and `bin/resolve-route.py`. Domain files load from that same checkout by domain: `mcp-routing.md` · `sol-usage.md` · `cursor-usage.md` · `fireworks-usage.md` · `usage-metering.md` · `visual-qa.md` · `grokbot-connection.md` · `analytics-clarity.md` · `marketplace-intelligence.md` · `skills/README.md` · `skills/registry.json` · `EDGE-CASES.md`.
...
1. **Classify** by task class. Run the router instead of eyeballing prose:
   ...
   Before any connector/plugin-dependent route, run `python3 "${ORCA_REPO:-$HOME/git/mb-orchestration}/bin/detect-integrations.py" --refresh` to record a privacy-safe observation. Caller-created `--integration-session` input is integrity-only and cannot grant routing authority; do not pass it. Until the Codex product exposes a product-authenticated callable inventory issuer, connector-dependent dispatch parks. The connector config is only the authorization ceiling; unknown/stale/missing/disabled/removed/unhealthy/unregistered or observation-only access parks.
...
3. **Pick the implement seat** per `AGENTS.md` §Seats / the router's `--implement` plan: **Grok Build** by default · **GPT Terra** for Google-MCP bulk · **Sol/Opus** for MCP *judgment* only (`mcp-routing.md`). Storefront pixels → **Grok Build implements**; `bin/connectors.py` may render a Review D preview/live packet as preparation only. Do not pass it to `mb-review-d`: normal execution hard-parks before prompt reads until the code-owned pixel-input binding exists, then provider/route/browser/pixel gates and role tests must also pass (`visual-qa.md`; `visual-qa-cli.md`; `bin/grok-agent.py`). A renderable public live-audit packet is not an executable audit and never substitutes for preview review of unpublished pixels. Marketplace evidence → `marketplace-intelligence.md`: approved deposited inputs only; its CLI route remains hard-parked until the code-owned authorized-deposit manifest binding exists, then profile sync, live gates, and role tests must pass. Review D, Heat Map, and Marketplace Intelligence are review/input seats — **never implementers**. Legwork-or-stop: never dump volume on Sol/Opus/Cursor $.
...
6. **Route reviews by the router, not a fixed list.** It excludes authors, prefers a reviewer independent from the dispatcher, and uses Opus 4.8 only as a time-bounded Anthropic compatibility fallback — not the independent family after Sol. After Sol, Review E is the next independent-family slot only when wired/live/eligible. A dispatcher review is artifact-only. Cross-family requires two independence groups and unique physical invocations. Exhaustion / Review E / quota-vs-outage rules live in `AGENTS.md` + `fireworks-usage.md`.
