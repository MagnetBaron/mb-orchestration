...
3. **Pick the implement seat** per `AGENTS.md` §Seats / the router's `--implement` plan: **Grok Build** by default · **GPT Terra** for Google-MCP bulk · **Sol/Opus** for MCP *judgment* only (`mcp-routing.md`). Storefront pixels → **Grok Build implements, then** a config-rendered Review D visitor-preview Slack ticket; current public-site observation may use the separate config-rendered, exact-trigger, read-only live-audit ticket (`visual-qa.md`; `bin/connectors.py`). Live audit never substitutes for preview review of unpublished pixels and grants no mutation authority. Marketplace sold-price/competitor evidence → `marketplace-intelligence.md`: supplied approved snapshots or positively authorized API outputs only; its Bot route remains parked until app-created and test-run. Review D, Heat Map, and Marketplace Intelligence are review/input seats — **never implementers**. Legwork-or-stop: never dump volume on Sol/Opus/Cursor $.
