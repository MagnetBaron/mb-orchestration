...
1. **Classify** by task class. Run the router instead of eyeballing prose:
   ...
   Before any connector/plugin-dependent route, run `python3 "${ORCA_REPO:-$HOME/git/mb-orchestration}/bin/detect-integrations.py" --refresh`. If the intake can enumerate current callable tools, pass a minimum safe one-runtime JSON overlay with `--integration-session <file>` (or stdin `-`). Never persist the overlay. The connector config is only the authorization ceiling; unknown/stale/missing/disabled/removed/unhealthy/unregistered access parks.
