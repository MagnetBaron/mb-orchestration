...
Sol is **Review B** (OpenAI family) and a tested dispatch target, not a default implementer. When selected as intake, Sol dispatches; Opus becomes the primary independent reviewer. Sol may later review an artifact it did not author only as `artifact-only`, never as independent proof of its own dispatch intent/risk.
...
| Model | Job |
|-------|-----|
| Luna or Terra | Codex worker seats: MCP volume + coordination; tested dispatch targets when selected |
| Sol | Git-diff review and tested dispatch target; dispatcher provenance changes review scope |
| Grok Build | All implementation |
| Opus 5 via teamclaude | Extra frontier pass if needed; not a Sol substitute for every ticket |
...
- At or over 90%: still dispatch **Grok** for volume. Code review → **Opus 5**, then **Review E (Fireworks) if wired**, otherwise park. Do not fake a “review” on Terra/Luna.
...
## Review check before starting Sol

1. Review brief with `attack_angle` and a git diff path?
2. Sol is not an implementer/author on this change-set?
3. Risk gate or owner requires review?
4. Weekly Sol under the soft cap per `usage-status`?
5. If Sol dispatched this run, is another reviewer independently validating dispatch intent/risk?
