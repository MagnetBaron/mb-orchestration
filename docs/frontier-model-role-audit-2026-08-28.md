...
Every `live_verified` route carries an `attestations` object covering `intake.promote_requires` (`official_id`, `local_access_smoke`, `role_evals`, `independent_evidence`, `cost_context`, `owner_approval`). Each entry has a typed `state`: `attested`, `missing`, `not_applicable`, or `waived`. Boolean `attested: true` is rejected. `attested` requires a dated direct supporting source whose semantics support that requirement; text such as “no suite,” “not invented,” “missing,” or “not available” cannot pass. `missing` cannot be `live_verified`. `not_applicable` is structural inapplicability with rationale, never a synonym for missing. `waived` is a time-bounded legacy/standing-provider migration exception (`authority: existing_operational_state`, date, short expiry). It is visible in inventory/matrix/report and does **not** assert that the evidence exists. After expiry the route fails closed. New candidates may never use a legacy waiver. No committed owner-approval document exists, so owner_approval is waived on existing operational seats rather than claimed as attested.

`local_access_smoke.signal` is either `direct_invocation` (Opus 5, Opus 4.8, Fable 5 teamclaude smokes) or `standing_provider` (Codex Sol/Terra/Luna listings, Grok Build listing, Cursor pool, Grok Bot identities). Standing seats were not given invented smokes. `official_id` attestations cite direct official vendor https URLs; local JSON paths are not sufficient.
...
| `opus-4.8-teamclaude` | teamclaude smoke (`OPUS48_SMOKE_OK`) | official_id **attested**. role_evals and independent_evidence **not_applicable** (compatibility fallback, not a ranked candidate). cost_context and owner_approval **waived** (expires 2026-11-26). |
...
| `grok-bot-visual-qa` | standing Review D provider | owner_approval, cost_context. role_evals **not_applicable** (pixel-walk app, not the text-model suite). independent_evidence **attested** on grok-4.6. |
| `grok-bot-heat-map` | standing Heat Map provider | owner_approval, cost_context. role_evals **not_applicable** (analytics-input app, not the text-model suite). independent_evidence **attested** on grok-4.6. |
...
4. Catalog new models as `unwired` or `catalog_verified`. Promote to `live_verified` only after official-id, local smoke, role evals, independent evidence, cost/context, and owner approval (`intake.promote_requires`) are **attested** with typed state. Each live field is `attested` (dated supporting source whose semantics match), `not_applicable` (structural, with rationale), or, for already-operational legacy seats only, `waived` (`existing_operational_state`, date, short expiry). `missing` cannot promote. New candidates may never use a legacy waiver. `official_id` requires a direct official https URL; local JSON paths are not sufficient. Record `local_access_smoke.signal` as `direct_invocation` or `standing_provider`. Do not invent new smokes; standing operational seats stay labeled standing. Review E remains a local placeholder until a named candidate plus official source exists.
5. Same-harness receipts are the path to promoting an external/operational prior into a measured role ranking. Do not label a quality row `local_same_harness` unless a same-role local receipt exists. Before claiming realized token-efficiency savings for context scouting, evidence audit, or model-evaluation admin, commit before/after token receipts (`token-eff-1` or equivalent). The architecture receipt is not that evidence.
