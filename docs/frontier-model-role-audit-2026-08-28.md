...
- `bin/model-registry.py resolve` is fail-closed: it filters every candidate with `route_is_live` and does not depend on CLI `assert_valid`. Missing, stale, future, mismatched, or unattested evidence, or a route-local identity/family/invocation contradiction, never returns the route.
