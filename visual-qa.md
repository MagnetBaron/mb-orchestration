...
3. **Preview mode:** accept only (a) the configured preview-host pattern with the ticket's configured site, or (b) a per-store event filter whose token is present in the ticket URL, whose host exactly equals that site's configured live host, and whose required `preview_theme_id` query parameter has a non-empty value. A bare or different live host, a missing/empty query parameter, or an unregistered host-prefix filter → `blocked: need a fresh Share Preview`. A preview may add to cart only to reach the cart page; it never opens or submits checkout or any other form.
