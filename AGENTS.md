...
- `config/handoff-policy.json` preauthorizes minimum-necessary ordinary repo artifacts between configured providers under `standing_review_authorization` (all configured review providers; no per-review approval). The intake family may review only as a separate physical invocation, `artifact-only`, and never as the sole reviewer of dispatch intent/risk. Authorship never creates a permission prompt. The immutable minimum in `bin/handoff_policy.py` keeps credentials, secrets/tokens, restricted PII/data, customer data, and production data/exports restricted even if config tries to remove them or list them as ordinary; restricted wins every conflict. Restricted and unknown classes park without a permission loop.
