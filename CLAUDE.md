...
- **Fable 5: review / architecture only; never the default implementer.** Fable is a per-account grant — available only while a live Claude seat includes it. Confirm with `bin/detect-fable.py`; a downgrade is recorded to the ledger and `bin/resolve-route.py` re-routes automatically.
