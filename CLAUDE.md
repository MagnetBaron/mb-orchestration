...
- **Fable 5: review / architecture only; never the default implementer.** Fable is a per-account grant — available only while a live Claude seat includes it. Confirm with `bin/detect-capability.py`; a downgrade is recorded to the ledger and `bin/resolve-route.py` re-routes automatically.
...
- After downgrade to Pro / Team-Standard: no included Fable — review order collapses to **Sol → Opus 4.8 → Review E (if wired) → stop**. teamclaude `mb/sync-plan` blocks `*fable*` automatically when no seat can serve it; do not hand-edit routes.
