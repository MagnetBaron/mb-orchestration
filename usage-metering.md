...
- `bin/record-429.sh` validates the seat against `usage-windows.json` before any write, holds a same-directory lock, writes a private temporary file, and atomically renames it into place. An unknown seat or malformed ledger fails without replacing the ledger.
