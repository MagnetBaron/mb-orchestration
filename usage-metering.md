...
- A metered monthly cap is usable only with finite, non-negative spend evidence whose `monthly_spend_period` matches the current `YYYY-MM` billing period and whose `updated` timestamp falls inside that period. Missing, malformed, future, or stale spend parks the seat.
