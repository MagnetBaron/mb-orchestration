...
Docs: event triggers are Slack/GitHub via Cursor account integrations, not the plugin tile. The event matcher is contains-only, so create one narrow event routine per config-rendered token and make the Bot re-check the complete ticket before it navigates. Website Visual QA still has two modes, but preview mode can need more than one event filter. Never use “every new message” or one broad OR routine.
...
Required for a shared-domain preview routine: channel is `#visual-qa` and the body contains the
configured shared-preview token plus a site name on the allowlist. A configured live-host theme
preview uses its own exact-host-prefix event token and must also pass exact host +
`preview_theme_id` checks. Render `visual-qa-allowlist` to see every current event token; render
`visual-qa-ticket <store>` to prove each store's actual ticket contains one.
...
5. Create one routine for **each config-rendered event token** owned by Website Visual QA. The
platform cannot safely express these alternatives in one contains filter. Today that means the
shared-preview token, each configured exact-host theme-preview token, and the live-audit token.
...
1. Message is in `#visual-qa` and contains exactly one configured event token for its mode/store.
...
5. The routine for that exact event token is enabled and its in-Bot field/host/query checks pass.
...
- Combining preview filters or preview/live audit into an unsafe broad routine
