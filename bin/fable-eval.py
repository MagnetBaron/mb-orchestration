...
Fable is being measured for a **GENERATION home only.** Fable is already OUT of the
gating/refusal order and out of dispatch — it is a rare architecture/long-horizon
escalation in the same Anthropic family as Opus 5. These trials say nothing about
gating and cannot put it back into the gate. They only ask: does Fable *generate*
well enough on some axis to earn a NON-gating seat? A seat is earned on an axis
only where Fable **clearly beats** Opus 5 (mean gap >= 1.5 / 10 AND a majority of
paired runs). A tie or a loss = no seat. Do not cite unaudited historical scores.
