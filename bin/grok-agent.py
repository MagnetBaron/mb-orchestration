...
def _render(recipe: dict, *, cwd: Path, prompt_file: Path) -> list[str]:
    context = {"repo": str(cwd), "brief_path": str(prompt_file)}
...
def inspect(seat: str, cwd: Path, prompt_file: Path | None, agent_dir: Path) -> dict:
    ...
    if not agent or not profile or not profile.is_file():
        problems.append(f"required Grok agent profile {agent!r} is not installed")
    elif f"name: {agent}" not in profile.read_text(errors="replace"):
        problems.append(f"installed profile {profile} has the wrong agent name")
    if prompt_file is None or not prompt_file.is_file():
        problems.append("prompt file does not exist")

    argv = _render(recipe, cwd=cwd, prompt_file=prompt_file or Path("<prompt-file>"))
    if argv[0] != "grok":
        problems.append("recipe binary must be exact grok")
    expected = [
        "--cwd", str(cwd), "--agent", str(agent), "--prompt-file",
...
def main(argv=None) -> int:
    ...
    if args.smoke:
        ...
        cmd = ["grok", "--cwd", str(cwd), "--agent", agent, "--model", "grok-4.6",
               "--reasoning-effort", "high", "--no-subagents", "--output-format", "plain",
