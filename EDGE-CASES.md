...
| Fable missing (downgrade) | `bin/detect-capability.py` records `fable-downgrade:<seat>`; `resolve-route` drops it. When NO Fable-capable seat is live, review order starts at **Codex Sol**, then Opus 4.8 (on any live Claude seat), then **Review E (if wired)**, else park after 4.8. |
...
deleted model ID; `bin/doctor.py` fails if any provider selects a forbidden model (Opus 5.0 — matched exactly by `mborch.is_opus5_zero`; 5.1+ allowed). Once
