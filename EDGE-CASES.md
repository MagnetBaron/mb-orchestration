...
| Opus / all Claude seats exhausted | Sol if under its soft cap (`usage-status`) and not already used on this change-set. Else park review — or, **if Review E is wired**, engage it only when the brief is time-critical and `usage-status` shows all native seats spent (`fireworks-usage.md`); its `ship` on a risk class is advisory, owner lands. Unwired → park after Opus 5. |
...
| Review D CLI/profile/browser prerequisite absent | `bin/grok-agent.py` reports PARK. Do not treat CLI smoke or WebFetch as pixels; park only Review D while unrelated implementation may continue. |
