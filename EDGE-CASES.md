...
| Opus / all Claude seats exhausted | Sol remains usable even at its soft cap when needed and when it was not already used on this change-set. If one native pass is insufficient, **Review E (if wired)** may fill the independent-family slot under the exact exhaustion/time-critical rules in `fireworks-usage.md`; its `ship` on a risk class is advisory, owner lands. Review E unwired → keep the valid Sol pass and park only the still-unsatisfied gate. |
